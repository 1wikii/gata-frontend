...
// Fetch dosen profile (current logged-in user)
export const fetchDosenProfile = async (): Promise<AdminProfile> => {
...
export const fetchExpertiseGroups = async (): Promise<ExpertiseGroup[]> => {
  const response = await api.get("/dosen/profile/kelompok-keahlian");
  if (!response.ok) {
 ...
 *
 * Updates dosen profile information including personal data, expertise groups, password, and signature.
 *
 * @param formData - Dosen profile form data with all required and optional fields
 * @returns Updated AdminProfile with all fields including signature
 * @throws Error if API request fails or validation error
 *
 */
export const updateDosenProfile = async (
  ...
  const response = await api.put("/dosen/profile", payload);
